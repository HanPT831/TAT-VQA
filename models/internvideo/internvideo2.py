...
class PretrainInternVideo2(nn.Module):
    def __init__(
        ...
        self.depth = depth
        self.fuse_layer_indices = [24, depth - 1]
        self.clip_norm_type = clip_norm_type
        self.return_index = []
        ...
        # self.layer_fusion_logits = nn.Parameter(torch.zeros(len(self.fuse_layer_indices)))
    ...
    def _fuse_hidden_states(self, hidden_list):
        # weights = torch.softmax(self.layer_fusion_logits, dim=0)  # [n_layers]
        weights = [0.2, 0.8]
        x = 0.
        for w, h in zip(weights, hidden_list):
            x = x + w * h
        return x
    @property
    def dtype(self):
    ...
    # @torch.cuda.amp.autocast(enabled=False)
    def forward(self, x, use_image=False, x_vis_return_idx=-1, x_vis_only=True):
        ...
        # keep old behavior when user explicitly asks for one intermediate layer
        # do_fusion = (x_vis_return_idx == -1)
        do_fusion = False

        fused_states = {}
        fuse_layer_set = set(self.fuse_layer_indices) if do_fusion else set()
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if do_fusion:
                if idx in fuse_layer_set:
                    fused_states[idx] = self._to_hidden(x)
            else:
                if idx == (self.depth + x_vis_return_idx):
                    break

        if do_fusion:
            missing = [i for i in self.fuse_layer_indices if i not in fused_states]
            if len(missing) > 0:
                raise ValueError(
                    f"Some fusion layers were not reached: {missing}. depth={self.depth}"
                )

            hidden_list = [fused_states[i] for i in self.fuse_layer_indices]
            x = self._fuse_hidden_states(hidden_list)
        else:
            x = self._to_hidden(x)

        x_vis = x[:, 1:, :]
        if x_vis_only:
            x_vis = x_vis.view(B, T, L, C)
            return x_vis
...
def pretrain_internvideo2_1b_patch14_224(config):
    ...
    if config.vision_encoder.pretrained is not None:
        ...
        message = model.load_state_dict(state_dict["model"], strict=False)
        logger.info(message)
    else:
...
def build_internvideo(config):
    model = PretrainInternVideo2(
        in_chans=3, 
        img_size=config.vision_encoder.img_size, 
        patch_size=config.vision_encoder.patch_size,
        embed_dim=config.vision_encoder.encoder_embed_dim, 
        depth=config.vision_encoder.encoder_depth, 
        num_heads=config.vision_encoder.encoder_num_heads, 
        mlp_ratio=48/11,
        clip_embed_dim=config.vision_encoder.clip_embed_dim,
        attn_pool_num_heads=16, qkv_bias=False,
    ...
    if config.vision_encoder.get("pretrained", None) is not None:
        logger.info(f"Loading pretrained weights from {config.vision_encoder.pretrained}")
        checkpoint_model_dict = torch.load(config.vision_encoder.stage1, map_location='cpu')
        checkpoint_model = checkpoint_model_dict["module"]
        if config.vision_encoder.pretrained is not None:
            state_dict = torch.load(config.vision_encoder.pretrained, map_location='cpu')
            extra_checkpoint_model = state_dict['module']
            ...
            interpolate_pos_embed_internvideo2(checkpoint_model, model, orig_t_size=4)
            msg = model.load_state_dict(checkpoint_model, strict=False)
            logger.info(msg)
            logger.info("Load Stage2")
    elif config.vision_encoder.stage1 is not None:
        logger.info(f"Loading pretrained weights from {config.vision_encoder.stage1}")
        checkpoint_model_dict = torch.load(config.vision_encoder.stage1, map_location='cpu')
        checkpoint_model = checkpoint_model_dict["module"]
        interpolate_pos_embed_internvideo2(checkpoint_model, model, orig_t_size=8)
        msg = model.load_state_dict(checkpoint_model, strict=False)
        logger.info(msg)
        logger.info("Load Stage1")
    else:
        logger.info("No pretrained weights!!!")
    return model
